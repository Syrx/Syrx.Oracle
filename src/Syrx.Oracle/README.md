...
> [!CAUTION]
> Oracle doesn't natively support multiple result sets in the same way that SQL Server and other databases do. As a result of this difference in the handling of multiple result sets, repositories written against Oracle instances may need code changes either when switching to Oracle or switching away from Oracle.

### Known Issues

Oracle's handling of multiple result sets differs significantly from other RDBMS implementations. However, it **is** possible to approximate this behavior using cursors.

For example, consider this PL/SQL:
...
Executing this SQL via `QueryAsync<>` or `Query<>` may return an `OracleException` with an error similar to:
...
The solution requires additional setup in your repository code:

1. Add a reference to `Syrx.Commanders.Databases.Oracle`
2. Pass a static instance of `OracleDynamicParameters.Cursors()` method as part of your parameters

We recommend leveraging the `using static` language feature for cleaner code.
...
// Assuming a Func<> delegate called 'map' on a method that would not normally need parameters 
var result = await _commander.QueryAsync(map, Cursors());
```

#### With Parameters - Numbered Cursors
...
var arguments = new { id1 = 2, id2 = 3, id3 = 4 };  // Normal parameters
var parameters = Cursors(arguments);                // Create OracleDynamicParameters
var result = await _commander.QueryAsync(map, parameters);
```

#### With Parameters - Named Cursors

For cases where you need to specify cursor names explicitly:

```sql
BEGIN
    OPEN :by_id    FOR select cast(id as number(5)) as "Id", name as "Name" from poco where id < :id;
    OPEN :by_name  FOR select cast(id as number(5)) as "Id", name as "Name" from poco where name like :name;
    OPEN :by_value FOR select cast(id as number(5)) as "Id", name as "Name" from poco where value < :value;
END;
```
...
public async Task<Employee> CreateEmployeeAsync(Employee employee)
{
    try
    {
        return await _commander.ExecuteAsync(employee) ? employee : default;
    }
